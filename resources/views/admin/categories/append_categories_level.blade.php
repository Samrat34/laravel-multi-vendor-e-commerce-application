    ...
    <select name="parent_id" id="parent_id" class="form-control" style="color: #000">
        ...
        @if (!empty($getCategories))
            ...
            @foreach ($getCategories as $parentCategory)
                {{-- Show the Categories --}} {{-- $getCategories are all the parent categories, and their child categories --}}
                @php
                ...
                @if (!empty($parentCategory['subCategories']))
                    {{-- Using the hasMany relationship in Category.php Model --}}
                    @foreach ($parentCategory['subCategories'] as $subcategory)
                        {{-- Show the Subcategories --}}
                        <option value="{{ $subcategory['id'] }}" @if (isset($subcategory['parent_id']) && $subcategory['parent_id'] == $subcategory['id']) selected @endif>
                            &nbsp;&raquo;&nbsp;{{ $subcategory['category_name'] }}</option> {{-- https://www.w3schools.com/charsets/ref_html_entities_r.asp --}}
                    @endforeach
                @endif
            @endforeach
