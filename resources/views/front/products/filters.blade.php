                ...
                <div class="associate-wrapper">
                    ...
                    @foreach ($getSizes as $key => $size)
                        ...
                        <input type="checkbox" class="check-box size" id="size{{ $key }}" name="size[]"
                            value="{{ $size }}"> {{-- Note!!: PLEASE NOTE THE SQUARE BRACKETS [] OF THE "name" ATTRIBUTE!! --}} {{-- echo the $size as a 'CSS class' to be able to use it in jQuery for filtering --}}
                        {{-- the checked checkboxes <input> fields of the size filter values (like Small, medium, large, XL, ...) will be submitted as an ARRAY because we used SQUARE BRACKETS [] with the "name" HTML attribute in the checkbox <input> field in filters.blade.php, or else, AJAX is used to send the <input> values WITHOUT submitting the <form> at all --}}
            ...
            <form class="facet-form" action="#" method="post">
                <div class="associate-wrapper">
                    ...
                    @foreach ($getColors as $key => $color)
                        {{-- show the correct relevant product 'color' filter values (e.g. for the 'men' category (red, blue, ...) BUT for the mobiles category (grey, black, ...)) depending on the URL --}}
                        <input type="checkbox" class="check-box color" id="color{{ $key }}" name="color[]"
                            value="{{ $color }}"> {{-- Note!!: PLEASE NOTE THE SQUARE BRACKETS [] OF THE "name" ATTRIBUTE!! --}} {{-- echo the $color as a 'CSS class' to be able to use it in jQuery for filtering --}}
                        {{-- the checked checkboxes <input> fields of the color filter values (like red, blue, ...) will be submitted as an ARRAY because we used SQUARE BRACKETS [] with the "name" HTML attribute in the checkbox <input> field in filters.blade.php, or else, AJAX is used to send the <input> values WITHOUT submitting the <form> at all --}}
            ...
            <form class="facet-form" action="#" method="post">
                <div class="associate-wrapper">
                    ...
                    @foreach ($getBrands as $key => $brand)
                        {{-- show the correct relevant product 'brand' filter values (e.g. for the 'men' category (LC Waikiki, Concrete, ...) BUT for the mobiles category (iPhone, Xiaomi, ...)) depending on the URL --}}
                        <input type="checkbox" class="check-box brand" id="brand{{ $key }}" name="brand[]"
                            value="{{ $brand['id'] }}"> {{-- Note!!: PLEASE NOTE THE SQUARE BRACKETS [] OF THE "name" ATTRIBUTE!! --}} {{-- echo the $brand as a 'CSS class' to be able to use it in jQuery for filtering --}}
                        {{-- the checked checkboxes <input> fields of the brand filter values (like Concrete, Zara, ...) will be submitted as an ARRAY because we used SQUARE BRACKETS [] with the "name" HTML attribute in the checkbox <input> field in filters.blade.php, or else, AJAX is used to send the <input> values WITHOUT submitting the <form> at all --}}
                ...
                <div class="associate-wrapper">
                    ...
                    @foreach ($prices as $key => $price)
                        <input type="checkbox" class="check-box price" id="price{{ $key }}" name="price[]"
                            value="{{ $price }}"> {{-- Note!!: PLEASE NOTE THE SQUARE BRACKETS [] OF THE "name" ATTRIBUTE!! --}} {{-- echo the $price as a 'CSS class' to be able to use it in jQuery for filtering --}}
                        {{-- the checked checkboxes <input> fields of the price filter values (like '1000-2000', '2000-5000', ...) will be submitted as an ARRAY because we used SQUARE BRACKETS [] with the "name" HTML attribute in the checkbox <input> field in filters.blade.php, or else, AJAX is used to send the <input> values WITHOUT submitting the <form> at all --}}
                        <label class="label-text" for="price{{ $key }}">BDT {{ $price }}
                        </label>
                    @endforeach
                </div>
            </form>
...
$filterAvailable = \App\Models\ProductsFilter::filterAvailable(
    ...
    $categoryDetails['categoryDetails']['id'],
            ...
            @if ($filterAvailable == 'Yes')
                {{-- if the current category has a filter --}}
